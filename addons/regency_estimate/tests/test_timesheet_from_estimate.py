...
class TimesheetFromEstimate(crm_common.TestCrmCommon):
    ...
    def test_01(self):
        ...
        self.assertEqual(len(price_sheet.mapped('item_ids')), 2)
        ...
        estimate.action_new_price_sheet()
        self.assertEqual(len(price_sheet), 1)
        self.assertEqual(len(price_sheet.mapped('item_ids')), 4)
        self.assertEqual(len(estimate.mapped('product_lines')), 2)
        ...
        # purchase_requisition.write({'line_ids': [(4, pr_new_line.id)]})
        estimate.product_lines.write({'selected': True})
        estimate.action_new_price_sheet()
